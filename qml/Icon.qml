...
Item {
    ...
    // Symbols that carry an outlined drawing for their resting state, and a
    // semibold one for a selected toggle that has no filled form. Cohort's
    // symbols are all drawn filled (FILL=1), so neither applies yet; the
    // layering stays for the day one does.
    readonly property bool hasOutline: false
    readonly property bool hasSemibold: false
